...
# ------------------ LOGIN FEATURE -------------------
# ======================================================
# Minimal in-memory stores for demo
...
async def send_otp(email: str = Form(...)):
    ...
    # TEMP: log OTP (replace with SMTP in prod)
...
async def verify(email: str = Form(...), otp: str = Form(...)):
    ...
    SESSION_STORE[token] = {"email": email, "created": time.time()}
...
async def analyze_resumes(
    ...
    logger.info(f"👤 Request by user: {user_email}")
